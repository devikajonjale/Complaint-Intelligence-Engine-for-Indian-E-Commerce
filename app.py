...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    quick_start_box([
        ...
        "**Churn proxy model (pipeline):** Features are **score**, **log(thumbs_up)**, **review_length**, **is_hinglish**, and **route_confidence** — deliberately **excluding** severity and anomaly scores so the label is not leaked from the same signals. The training label flags complaint-bucket reviews that are **very low-star (≤1)** or in the **top 20% by review length** (engagement / frustration proxy). Metrics use a **platform group holdout** to reduce same-corpus overfitting.",
